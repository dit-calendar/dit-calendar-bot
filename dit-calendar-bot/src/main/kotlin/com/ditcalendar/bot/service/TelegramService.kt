...
class TelegramService(private val ditCalendarService: DitCalendarService,
    ...
    fun callbackRequestParser(request: String, originallyMessage: IncomingMessage, msgUser: User): OutgoingCallbackQueryMessage =
            if (request.startsWith(unassignCallbackCommand)) {
                val opts = request.removePrefix(unassignCallbackCommand)
                val telegramLink = TelegramLink(originallyMessage.chat.id.toLong(),
                        msgUser.id, msgUser.username, msgUser.firstName)
                val response = ditCalendarService.executeUnassignCallback(telegramLink, opts)
                buildCallBackResponseMessage(response, originallyMessage)

            } else
                OutgoingCallbackQueryMessage().apply { text = wrongRequestMessage }

    private fun editOriginalMessage(originallyMessage: IncomingMessage, newText: String) {
    ...
    private fun buildCallBackResponseMessage(response: Result<Base, Exception>, originallyMessage: IncomingMessage): OutgoingCallbackQueryMessage {
        ...
        when (val result = stringFormatter.parseResponse(response)) {
            is OnlyText -> {
                ...
                response.success {
                    editOriginalMessage(originallyMessage, result.message)
                    callbackResponse.text = successfullyUnassignedMessage
                }
            }
            is WithInline -> {
                callbackResponse.showAlert = true
                callbackResponse.text = result.message
            }
        }
    ...
    companion object {
        ...
        private const val unassignCallbackCommand = "unassign_"
    }
}